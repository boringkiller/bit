...
public class DealCostServiceImpl implements DealCostService {
	...
	public boolean payAndReceiveDealCost(Deal deal) {
		try {
			...
			balanceService.reduceMoneyBalance(buyer, MathUtils.add(totalAmount, moneyCost)); //付出现金
			balanceService.increaseBtcBalance(buyer, Double.valueOf(deal.getQuantity()));
			balanceService.reduceBtcBalance(seller, MathUtils.add(quantity, btcCost)); //付出比特币
			balanceService.increaseMoneyBalance(seller, totalAmount);
